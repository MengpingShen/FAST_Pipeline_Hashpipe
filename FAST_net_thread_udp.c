...
//    raw_header = le64toh(*(unsigned long long *)p->data);
    memcpy(&raw_header,p->data,N_BYTES_HEADER*sizeof(char));
    printf("Header raw: %lu \n",raw_header);
    pkt_header->mcnt        =  raw_header >> 8	& 0x00ffffffffffffff;
...
static inline uint64_t process_packet(FAST_input_databuf_t *db, struct hashpipe_udp_packet *p)
{
...
//    uint64_t dest_p	= 0;
	...
	printf("\n ********start !!!******\n\n");
        if (total_packets_counted == 0 ){ binfo.cur_mcnt = pkt_mcnt;}
...
//            payload_p = (uint64_t *)(p->data+8);
            memcpy((db->block[binfo.block_idx].data)+binfo.offset, p->data+8, DATA_SIZE_PACK*sizeof(char));
...
//        hputi8(st.buf, "RCVMB", binfo.rcvmb);
 ...
 //       p.packet_size = recv(up.sock,p.data,PKTSIZE,0);
	...
	if (up.packet_size == PKTSIZE){
		const uint64_t netmcnt = process_packet((FAST_input_databuf_t *)db, &p);
	}
        pthread_testcancel();
     }// Main loop
     return THREAD_OK;
